      ...
      >
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            <TrainAvatar number={favorite.trainNumber} size="md" />
            ...
            <div className="flex items-center gap-2.5 rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                  RailRadar se live data mil nahi paya â€” koi fake status nahi dikhaya ja raha.
                </p>
              </div>
            </div>
          ) : journey ? (
            <>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <LiveBadge state={liveStateFromTimestamp(journey.lastUpdated)} />
                </div>
      ...
      <div className="space-y-8 py-4">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <PageHeader
            ...
            description={
              favorites.length > 0
                ? `${favorites.length} saved train${favorites.length !== 1 ? 's' : ''} Â· live status from RailRadar`
                : 'Apni favourite trains ek jagah â€” live status ke saath'
            }
          />
          {favorites.length > 0 && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-600 shadow-sm hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors disabled:opacity-50"
            >
